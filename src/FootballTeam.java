    ...
    private void validateTactic(String tactic) {
        if (!VALID_TACTICS.contains(tactic)) {
            throw new IllegalArgumentException("Invalid football tactic: " + tactic +
                    ". Allowed tactics: " + VALID_TACTICS);
        }
    }

    public void addPlayerToRoster(FootballPlayer player) {
        if (getAllPlayers().size() >= MAX_ROSTER_SIZE) {
            throw new IllegalStateException("Cannot add player. Football team roster is capped at " +
                    MAX_ROSTER_SIZE + " players.");
        }
    ...
    public void setMatchDayLineup(List<AbstractPlayer> starters, List<AbstractPlayer> bench) {
        if (starters.size() != PLAYERS_ON_FIELD) {
            throw new IllegalArgumentException("A football match requires exactly " +
                    PLAYERS_ON_FIELD + " starting players.");
        }
        if (starters.size() + bench.size() > MAX_ROSTER_SIZE) {
            throw new IllegalArgumentException("Match day squad exceeds the maximum allowed roster size of " +
                    MAX_ROSTER_SIZE + ".");
        }
    ...
    public String toString() {
        return String.format("FootballTeam{name='%s', coach='%s', tactic='%s', rosterSize=%d}",
